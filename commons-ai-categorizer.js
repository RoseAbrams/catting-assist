...
(function () {
    ...
    function buildUi() {
        var fileEl = document.getElementById('file') || document.querySelector('.fullMedia');
        var host = document.getElementById('mw-imagepage-content') || document.getElementById('content') || document.body;

        var panel = document.createElement('div');
        panel.id = 'ai-categorizer-panel';
        panel.innerHTML = [
            '<div class="ai-cat-header">AI Category Assistant</div>',
    ...
    function randomUncategorizedUrl() {
        ...
        return mw.util.getUrl('Special:RandomInCategory/All media needing categories as of ' + year);
    }
    ...
    async function requestAiCategories(title, wikitext) {
        if (!CONFIG.aiEndpoint) {
            throw new Error('AI endpoint not configured. Set CONFIG.aiEndpoint first.');
        }
        ...
        var body = {
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are a Wikimedia Commons categorization assistant. Output only category wikimarkup lines.'
                },
                {
                    role: 'user',
    ...
    function normalizeAiOutputToCategoryLines(aiText) {
        ...
        if (!cats.length) {
            lines.forEach(function (line) {
                var plain = line.replace(/^[-*\d.\s]+/, '').replace(/^Category\s*:\s*/i, '').trim();
                if (plain) {
    ...
    async function onPrimaryClick(ui) {
        ...
        if (state.mode === 'generate') {
            setWorking(ui, true, 'Collecting page wikitext and generating categories...');
            try {
                var pageData = await fetchCurrentPageWikitext();
                var aiRaw = await requestAiCategories(pageData.title, pageData.wikitext);
                ...
                setWorking(ui, false, 'Generate failed: ' + (err && err.message ? err.message : String(err)));
            }
            return;
        }
        ...
        if (!userText) {
            ui.status.textContent = 'Nothing to submit. Add at least one category line.';
            return;
        }
        ...
        try {
            ...
            setWorking(ui, false, 'Submit failed: ' + (err && err.message ? err.message : String(err)));
        }
    }
